...
export default function ExpiredBanner({ sessionId }: Props) {
  const { sessions, renameSession, deleteSession } = useSessionStore();
  ...
  const handleCatchUp = useCallback(async () => {
    ...
    try {
      ...
      const res = await apiFetch('/api/session/restore-summary', {
        ...
        body: JSON.stringify({
          messages,
          domain_id: sessions.find((item) => item.id === sessionId)?.vertical ?? 'generic',
          provider_id: sessions.find((item) => item.id === sessionId)?.provider ?? 'hf-jobs',
          dataset_repo: sessions.find((item) => item.id === sessionId)?.dataset?.repoId ?? null,
        }),
      });
