...
function InlineApproval({
  ...
  const budgetBlock = useAgentStore(s => s.budgetBlocks[toolCallId]);
  ...
  return (
    <Box sx={{ px: 1.5, py: 1.5, borderTop: '1px solid var(--tool-border)' }}>
      ...
      {budgetBlock && (
        ...
        >
          <Typography sx={{ color: 'var(--text)', fontSize: '0.74rem', fontWeight: 700, mb: 0.25 }}>
            Cost approval required
          </Typography>
          <Typography sx={{ color: 'var(--muted-text)', fontSize: '0.7rem', lineHeight: 1.5 }}>
            Estimate: {moneyLabel(budgetBlock.estimatedCostUsd)} · Remaining cap: {moneyLabel(budgetBlock.remainingCapUsd)}
            {budgetBlock.reason ? ` · ${budgetBlock.reason}` : ''}
          </Typography>
        </Box>
      )}
