...
import SessionFlowPanel from '@/components/SessionFlow/SessionFlowPanel';
import { domainPromptFromList, useDomainTemplates } from '@/components/SessionFlow/domainOptions';
...
export default function SessionChat({ sessionId, isActive, onSessionDead }: SessionChatProps) {
  ...
  return (
    <>
      <SessionFlowPanel sessionId={sessionId} />
      <MessageList
        messages={messages}
        isProcessing={busy}
        ...
        <ChatInput
          ...
          placeholder={
            activityStatus.type === 'waiting-approval'
              ...
              : domainPromptFromList(domainTemplates, session?.vertical ?? session?.domain_id)
          }
        />
      )}
    </>
