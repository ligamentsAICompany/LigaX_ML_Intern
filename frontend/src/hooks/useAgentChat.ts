...
import { sanitizeAutoFineTuneJobUrl, sanitizeAutoFineTuneModelUrl } from '@/utils/autoFineTuneResult';
import { useAgentStore } from '@/store/agentStore';
import { useSessionStore } from '@/store/sessionStore';
import { useLayoutStore } from '@/store/layoutStore';
import { logger } from '@/utils/logger';
import type { AutoFineTuneProgress, CloudProviderId, DomainId } from '@/types/agent';
...
function isProviderId(value: unknown): value is CloudProviderId {
  return ['hf-jobs', 'aws', 'azure', 'gcp'].includes(String(value));
}
...
function asAutoFineTuneProgress(data: unknown, previous?: AutoFineTuneProgress): AutoFineTuneProgress {
  const record = asRecord(data) ?? {};
  const state = asString(record.state) ?? previous?.state ?? 'resolving_dataset';
  return {
    ...previous,
    state,
    message: asString(record.message) ?? previous?.message ?? 'Auto fine-tune is running',
    provider_id: 'hf-jobs',
    approval_required: false,
    credential_readiness: asBooleanRecord(record.credential_readiness) ?? previous?.credential_readiness,
    model_repo_url: sanitizeAutoFineTuneModelUrl(asString(record.model_repo_url) ?? asString(record.model_url)) ?? previous?.model_repo_url,
    job_url: sanitizeAutoFineTuneJobUrl(asString(record.job_url)) ?? previous?.job_url,
    eval_result: asString(record.eval_result) ?? previous?.eval_result,
    estimated_total_cost_usd: asNumber(record.estimated_total_cost_usd) ?? previous?.estimated_total_cost_usd,
    cost_cap_usd: asNumber(record.cost_cap_usd) ?? previous?.cost_cap_usd ?? 5,
    attempt: asNumber(record.attempt) ?? previous?.attempt,
    max_attempts: asNumber(record.max_attempts) ?? previous?.max_attempts,
    error_code: asString(record.error_code) ?? previous?.error_code,
    updatedAt: new Date().toISOString(),
  };
}

function applyBackendSessionMetadata(sessionId: string, info: Record<string, unknown>) {
  const store = useSessionStore.getState();
  if (isDomainId(info.domain_id)) {
    store.updateSessionVertical(sessionId, info.domain_id);
  }
  if (isProviderId(info.provider_id)) {
    store.updateSessionProvider(sessionId, info.provider_id);
  }
  if (typeof info.dataset_repo === 'string' && info.dataset_repo.trim()) {
    const existingDataset = store.sessions.find((item) => item.id === sessionId)?.dataset;
    store.updateSessionDataset(sessionId, {
      ...existingDataset,
      status: 'ready',
      repoId: info.dataset_repo,
      url: `https://huggingface.co/datasets/${info.dataset_repo}`,
    });
  }
}
    ...
    () => ({
      ...
      onToolCallPanel: (toolName: string, args: Record<string, unknown>) => {
        if (toolName === 'hf_jobs' && args.operation && args.script) {
          const provenance = asRecord(args.provenance);
          const isAutoFineTune = provenance?.auto_finetune === true;
          if (isAutoFineTune) {
            const sessState = useAgentStore.getState().getSessionState(sessionId);
            updateSession(sessionId, {
              panelData: {
                title: 'Auto fine-tune',
                output: {
                  content: 'HF Jobs training is running. Progress and results are shown in the auto fine-tune timeline.',
                  language: 'markdown',
                },
                parameters: {
                  ...args,
                  autoFineTune: sessState.panelData?.parameters?.autoFineTune,
                },
              },
              panelView: 'output',
              panelEditable: false,
    ...
    (async () => {
      try {
        ...
        if (infoRes.ok) {
          ...
          applyBackendSessionMetadata(sessionId, info);
        ...
        while (true) {
          ...
          for (const line of lines) {
            ...
            try {
              ...
              } else if (et === 'auto_finetune_progress') {
                const message = typeof event.data?.message === 'string'
                  ? event.data.message
                  : 'Auto fine-tune is running';
                const sessState = useAgentStore.getState().getSessionState(sessionId);
                const progress = asAutoFineTuneProgress(
                  event.data,
                  sessState.panelData?.parameters?.autoFineTune as AutoFineTuneProgress | undefined,
                );
                updateSession(sessionId, {
                  activityStatus: { type: 'tool', toolName: 'hf_jobs', description: message },
                  panelData: {
                    title: 'Auto fine-tune',
                    output: {
                    ...
                    parameters: {
                      ...sessState.panelData?.parameters,
                      autoFineTune: progress,
                    },
                  },
                  panelView: 'output',
                  panelEditable: false,
