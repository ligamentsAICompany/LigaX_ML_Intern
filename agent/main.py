...
def _print_approval_metadata(tool_info: dict, event_data: dict | None = None) -> None:
    """Show policy metadata that explains why manual approval is required."""
    metadata = tool_info
    if event_data and not any(
        metadata.get(key) is not None
        for key in ("block_reason", "estimated_cost_usd", "remaining_cap_usd")
    ):
        metadata = event_data

    block_reason = metadata.get("block_reason")
    estimated_cost_usd = metadata.get("estimated_cost_usd")
    remaining_cap_usd = metadata.get("remaining_cap_usd")
    auto_approval_blocked = metadata.get("auto_approval_blocked")

    if auto_approval_blocked:
        print("Auto-approval blocked.")
    if block_reason:
        print(f"Reason: {block_reason}")
    if estimated_cost_usd is not None:
        print(f"Estimated cost: {_format_usd(estimated_cost_usd)}")
    if remaining_cap_usd is not None:
        print(f"Remaining auto-approval cap: {_format_usd(remaining_cap_usd)}")
        ...
        try:
            ...
            if event.event_type == "ready":
                ...
                if (
                    ...
                    and not event.data.get("auto_approval_blocked", False)
                ):
                    approvals = [
                        {
                ...
                for i, tool_info in enumerate(tools_data, 1):
                    ...
                    _print_approval_metadata(tool_info, event.data)
...
async def headless_main(
    ...
    while True:
        ...
        if event.event_type == "assistant_chunk":
            ...
            if not (event.data and event.data.get("auto_approval_blocked", False)):
                tools_data = event.data.get("tools", []) if event.data else []
                approvals = [
                    {
                        "tool_call_id": t.get("tool_call_id", ""),
