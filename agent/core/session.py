...
class Session:
    ...
    def __init__(
        ...
        self.sandbox = None
        self.provider_id: str = "hf-jobs"
        self._running_job_ids: set[str] = set()  # HF job IDs currently executing
        self.auto_approval_cost_cap_usd = self.config.auto_approval_cost_cap_usd
    ...
    async def send_event(self, event: Event) -> None:
        ...
        scrubbed_data = scrub(event.data or {})
        store = self.persistence_store
        if store is not None:
            try:
                event.seq = await store.append_event(
                    self.session_id, event.event_type, scrubbed_data
                )
            except Exception as e:
                logger.debug("Failed to persist event for %s: %s", self.session_id, e)
    ...
    def add_auto_approval_estimated_spend(self, amount_usd: float) -> None:
        """Reserve estimated billable spend for auto-approved actions."""
        self.auto_approval_estimated_spend_usd = round(
            self.auto_approval_estimated_spend_usd + float(amount_usd),
            4,
        )
