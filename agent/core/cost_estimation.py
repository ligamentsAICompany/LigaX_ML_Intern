"""Conservative cost estimates for billable infrastructure actions."""
...
# Fallback prices are intentionally conservative guardrails. A live HF Jobs
# hardware catalog overrides these values whenever it is reachable.
...
SANDBOX_PRICE_USD_PER_HOUR: dict[str, float] = {
    "cpu-basic": 0.0,
    "cpu-upgrade": 0.05,
    "cpu-performance": 0.50,
    "cpu-xl": 1.00,
...
class CostEstimate:
    ...
    ``estimated_cost_usd=None`` means the call may be billable but cannot be
    estimated safely, so approval policy must fall back to manual review.
    """
...
def parse_timeout_hours(
    ...
    """Parse HF-style timeout values into hours.

    Numeric values are seconds, matching `huggingface_hub`'s timeout parameter.
    Strings accept `s`, `m`, `h`, or `d` suffixes; a bare string is seconds.
    """
...
async def hf_jobs_price_catalog() -> dict[str, float]:
    ...
    prices = (
        {**HF_JOBS_PRICE_USD_PER_HOUR, **prices}
        if prices
        else dict(HF_JOBS_PRICE_USD_PER_HOUR)
    )
...
async def estimate_sandbox_cost(
    ...
    price = SANDBOX_PRICE_USD_PER_HOUR.get(hardware)
    if price is None:
        return CostEstimate(
            estimated_cost_usd=None,
            billable=True,
...
async def estimate_tool_cost(
    ...
    if tool_name == "hf_jobs":
        ...
        provider_id = str(
            ...
            or getattr(session, "provider_id", "hf-jobs")
            or "hf-jobs"
        )
        if provider_id != "hf-jobs" or operation in {
            "run",
            "uv",
            "scheduled run",
            "scheduled uv",
            ...
            try:
                provider = (
                    get_cloud_provider_registry().require_executable("hf-jobs")
                    if provider_id == "hf-jobs"
                    else get_cloud_provider_registry().require(provider_id)
                )
            except KeyError:
                return CostEstimate(
                    estimated_cost_usd=None,
                    billable=True,
                    block_reason=f"Unknown cloud provider: {provider_id}",
                    label=provider_id,
                )
            return await provider.estimate_cost(args)
