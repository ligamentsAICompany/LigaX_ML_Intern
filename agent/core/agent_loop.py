...
from agent.core.auto_finetune import AutoFineTunePipeline, is_auto_finetune_intent
from agent.core.approval_policy import (
    ApprovalDecision,
    decide_tool_approval,
    legacy_needs_approval,
...
_CALL_CENTER_FAST_PATH_HF_JOBS_ARGS = {
    "operation": "run",
...
def _call_center_fast_path_hf_jobs_tool_call() -> tuple[ToolCall, dict]:
    """Build the deterministic approval-only HF Jobs call for the smoke path."""
    args = dict(_CALL_CENTER_FAST_PATH_HF_JOBS_ARGS)
    return (
        ToolCall(
            id="functions.hf_jobs:phase9-fast-path",
            type="function",
            function={
                "name": "hf_jobs",
...
def _is_call_center_hf_jobs_fast_path(session: Session) -> bool:
    for item in reversed(session.context_manager.items):
        if item.role != "user" or not isinstance(item.content, str):
            continue
        content = item.content
        return (
            "Phase 6 approval-before-spend fast path" in content
            and "- Domain: Call Center" in content
            and "- Compute provider: HF Jobs" in content
        )
    return False


def _last_tool_was(session: Session, tool_name: str) -> bool:
    for item in reversed(session.context_manager.items):
        if item.role == "tool":
            return item.name == tool_name
        if item.role == "user":
            return False
    return False
...
def _needs_approval(
    ...
    """Legacy sync compatibility wrapper for approval checks."""
    return legacy_needs_approval(tool_name, tool_args, config)
...
async def _cleanup_on_cancel(session: Session) -> None:
    ...
    # Cancel running provider jobs through the selected provider adapter.
    ...
    if job_ids:
        ...
        try:
            ...
            logger.warning("Failed to cancel provider jobs on cancel: %s", e)
...
class LLMResult:
    ...
    usage_source: object | None = None
    latency_ms: int = 0


async def _prepare_approval_event_data(
    approval_required_tools: list[tuple[ToolCall, str, dict, ApprovalDecision]],
    *,
    session: Session,
) -> dict:
    """Build the existing approval_required event payload with optional policy metadata."""
    tools_data = []
    blocked_payloads = []
    for tc, tool_name, tool_args, decision in approval_required_tools:
        # Resolve sandbox file paths for hf_jobs scripts so the frontend can
        # display and edit the actual file content.
...
async def _record_manual_approved_spend_if_needed(
    ...
    decision = await decide_tool_approval(tool_name, tool_args, session=session)
    if decision.billable:
        record_estimated_spend(session, decision)
...
async def _call_llm_streaming(
    ...
    for _llm_attempt in range(_MAX_LLM_RETRIES):
        try:
            ...
            await telemetry.record_provider_error(
                session,
                provider=str(llm_params.get("model") or "unknown"),
                operation="llm_call",
                error=e,
            )
            raise
    ...
    await telemetry.record_llm_call(
        session,
        model=str(llm_params.get("model") or session.config.model_name),
...
class Handlers:
    ...
    async def run_agent(
        ...
        if text and is_auto_finetune_intent(text):
            result = await AutoFineTunePipeline(config=session.config).run(
                session, text, {}
            )
            if result.message:
            ...
            try:
                ...
                if not tool_calls:
                    if (
                        ...
                        and _is_call_center_hf_jobs_fast_path(session)
                        and _last_tool_was(session, "hf_inspect_dataset")
                    ):
                        tc, tool_args = _call_center_fast_path_hf_jobs_tool_call()
                        ...
                        if decision.requires_approval:
                            ...
                            session.pending_approval = {"tool_calls": [tc]}
                            return None
                ...
                # Reserved spend prevents one batch from exceeding the cap by
                # splitting a budget across multiple auto-approved calls.
