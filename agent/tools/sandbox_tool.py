...
it returns an explicit error so the agent requests sandbox_create first.
"""
...
def _make_tool_handler(sandbox_tool_name: str):
    ...
    async def handler(args: dict[str, Any], session: Any = None) -> tuple[str, bool]:
        ...
        if not session or not getattr(session, "sandbox", None):
            return (
                "No sandbox running. Call sandbox_create first to start one; "
                "sandbox creation requires approval before sandbox spend."
            ), False
