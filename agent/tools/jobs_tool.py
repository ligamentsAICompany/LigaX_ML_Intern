...
def _resolve_uv_command(
    ...
    # If it looks like inline Python, wrap it even when it is one line.
...
_SCRIPT_SMOKE_OPERATIONS = {"run", "uv", "scheduled run", "scheduled uv", "submit"}


def _blocked_script_smoke_result(smoke) -> ToolResult:
    return {
        "formatted": (
            "HF Jobs submission blocked before spend.\n\n"
            + format_script_smoke_result(smoke)
        ),
        "totalResults": 0,
        "resultsShared": 0,
        "isError": True,
...
def _script_smoke_preflight(
    args: Dict[str, Any],
) -> tuple[Dict[str, Any], ToolResult | None]:
    """Revalidate Python training script payloads at the final submit boundary."""

    operation = str(args.get("operation") or "").strip().lower()
    if operation not in _SCRIPT_SMOKE_OPERATIONS:
        return args, None
    ...
    if is_inline_python_script(script):
        ...
        smoke = unverified_script_smoke_result(script, job_args=args)
        if smoke is None:
            return args, None

    args = {**args, "script_smoke": smoke.to_dict()}
    if not smoke.passed:
        return args, _blocked_script_smoke_result(smoke)
    return args, None
...
class HfJobsTool:
    ...
    async def execute(self, params: Dict[str, Any]) -> ToolResult:
        ...
        args, blocked = _script_smoke_preflight(args)
        ...
        try:
            ...
            return {
                "formatted": f"API Error: {_safe_error_message(e)}",
                "totalResults": 0,
                "resultsShared": 0,
                "isError": True,
            }
            ...
            return {
                "formatted": f"Error executing {operation}: {_safe_error_message(e)}",
                "totalResults": 0,
                "resultsShared": 0,
                "isError": True,
            }
        ...
        for _ in range(max_retries):
            try:
                ...
                while True:
                    ...
                    # Process log line
                    log_line = scrub_string(str(item))
                ...
                try:
                    ...
                    logger.warning(
                        f"Connection interrupted ({_safe_error_message(e)[:50]}...), reconnecting in {retry_delay}s..."
                    )
...
```"""
            ...
            await telemetry.record_provider_error(
                self.session,
                provider="huggingface_jobs",
                operation="run_job",
                error=e,
            )
            raise Exception(f"Failed to run job: {_safe_error_message(e)}")
    ...
    async def _get_logs(self, args: Dict[str, Any]) -> ToolResult:
        ...
        try:
            ...
            log_text = scrub_string(_strip_ansi("\n".join(logs)))
            return {
            ...
            return {
                "formatted": f"Failed to fetch logs: {_safe_error_message(e)}",
                "isError": True,
                "totalResults": 0,
                "resultsShared": 0,
            }
    ...
    async def _inspect_job(self, args: Dict[str, Any]) -> ToolResult:
        ...
        for jid in job_ids:
            try:
                ...
                raise Exception(
                    f"Failed to inspect job {jid}: {_safe_error_message(e)}"
                )
...
To list all, call this tool with `{{"operation": "scheduled ps"}}`"""
            ...
            raise Exception(f"Failed to create scheduled job: {_safe_error_message(e)}")
...
HF_JOBS_TOOL_SPEC = {
    ...
    "description": (
        ...
        "- You MUST review the Trainability Gate result before direct fine-tuning. "
        "Pass `trainability` from inspection or `dataset_profile` with row_count, columns, format, sample_rows, "
        "missing_fraction, and duplicate_fraction when planning a training job. High-risk datasets recommended "
        "for rag/hybrid/data_needed are blocked for direct fine-tuning.\n"
        ...
        "- Training scripts MUST pass script smoke/preflight validation before spend: Python syntax, push_to_hub=True, hub_model_id, "
        "timeout guidance, and Trackio markers are checked where static validation can see them.\n"
        "- Include trackio monitoring and provide the dashboard URL to the user.\n"
        "- Include provenance metadata for auditable Hub artifacts: base model, dataset source/counts, "
        "strategy, trainability risk, golden eval summary, limitations, method, cost, hardware, and timeout.\n\n"
        ...
        "For selected AWS, Azure, or GCP sessions, use plan/validate or run as a safe dry-run plan. "
        "Do not use submit unless real provider execution has been explicitly enabled; Phase 10 blocks it by design.\n\n"
    ...
    "parameters": {
        ...
        "properties": {
            ...
            "trainability": {
                "type": "object",
                "description": (
                    "Trainability Gate result from dataset inspection, including score, recommendation, "
                    "risk_level, and reasons. High-risk non-fine_tune recommendations block direct training."
                ),
            },
            "dataset_profile": {
                "type": "object",
                "description": (
                    "Raw dataset suitability signals for Trainability Gate scoring when a precomputed result "
                    "is unavailable: row_count, columns, format, sample_rows, missing_fraction, duplicate_fraction."
                ),
            },
            "script_smoke": {
                "type": "object",
                "description": (
                    "Structured script smoke/preflight result. Failed smoke results block training spend; "
                    "the tool also computes this for inline or resolved sandbox Python scripts."
                ),
            },
            "preflight": {
                "type": "object",
                "description": "Alias for script_smoke metadata from external preflight validators.",
            },
            "trackio": {
                "type": "object",
                "description": (
                    "Trackio run metadata for training jobs, such as project, space_id, run_name, "
                    "and dashboard_url. Values are scrubbed before telemetry."
                ),
            },
            "provenance": {
                "type": "object",
                "description": (
                    "Hub artifact provenance payload including base_model, dataset summary, "
                    "trainability, strategy, golden_eval, limitations, and training method/cost/hardware/timeout."
                ),
            },
            "job_id": {
                "type": "string",
...
async def hf_jobs_handler(
    ...
    try:
        ...
        async def log_callback(log: str):
            if session:
                await session.send_event(
                    Event(
                        event_type="tool_log",
                        data={"tool": "hf_jobs", "log": scrub_string(str(log))},
                    )
                )
        ...
        arguments, blocked = _script_smoke_preflight(arguments)
        if blocked is not None:
            return blocked["formatted"], False

        provider_id = str(getattr(session, "provider_id", "hf-jobs") or "hf-jobs")
        hf_token = session.hf_token if session else None
        namespace = (
            os.environ.get("HF_NAMESPACE")
            or (HfApi(token=hf_token).whoami().get("name") if hf_token else None)
            if provider_id == "hf-jobs"
            else None
        )
        registry = get_cloud_provider_registry()
        provider = (
            registry.require_executable("hf-jobs")
            if provider_id == "hf-jobs"
        ...
        return f"Error executing HF Jobs tool: {_safe_error_message(e)}", False
